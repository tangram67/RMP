...
int32_t dyn_decomp( AGParamRecPtr params, BitBuffer * bitstream, int32_t * pc, int32_t numSamples, int32_t maxSize, uint32_t * outNumBits )
{
	...
	RequireAction( (bitstream != nil) && (pc != nil) && (outNumBits != nil), return kALAC_ParamError_80; );
	*outNumBits = 0;
    ...
    while (c < numSamples)
    {
    	...
    	RequireAction( bitPos < maxPos, status = kALAC_ParamError_81; goto Exit; );
        ...
        *outPtr++ = del;
        ...
        if (((mb << MMULSHIFT) < QB) && (c < numSamples))
        {
            ...
            //printf("ALAC:dyn_decomp() Samples = %d, c = %d, n = %d\n", numSamples, c, n);
            RequireAction(c+n <= numSamples, status = kALAC_ParamError_82; goto Exit; );
            if (c+n <= numSamples) {
				for(j=0; j < n; j++)
				{
					*outPtr++ = 0;
...
Exit:
	*outNumBits = (bitPos - startPos);
	...
	RequireAction( bitstream->cur <= bitstream->end, status = kALAC_ParamError_83; );
