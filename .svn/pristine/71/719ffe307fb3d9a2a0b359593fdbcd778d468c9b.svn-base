...
class ALACDecoder {
private:
	ALACSpecificConfig mConfig; // decoding parameters
...
protected:
	...
	int32_t * mMixBufferV;
	int32_t * mPredictor;
	uint16_t * mShiftBuffer; // Note: this points to mPredictor's memory but different variable for clarity and type difference

public:
	ALACSpecificConfig getConfig() const { return mConfig; };

	int32_t Init( const ALACSpecificConfig& config );
