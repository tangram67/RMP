...
ID3v2_tag* load_tag(const char* file_name)
{
    ...
    r = fread(buffer, header_size+10, 1, file);
    fclose(file);
...
ID3v2_tag* load_tag_with_buffer(char *bytes, size_t length)
{
    // Declaration
    ID3v2_frame *frame;
    size_t offset = 0;
    ...
    {
        frame = parse_frame(tag->raw, offset, get_tag_version(tag_header), length);
        if(frame != NULL)
        {
            offset += frame->size + 10;
            if (EF_INVALID_FRAME != get_frame_type(frame->frame_id)) {
            	add_to_list(tag->frames, frame);
            } //else
            	//printf("ID3v2:parse_frame() Unsupported frame \"%.*s\" \n", ID3_FRAME_SIZE, frame->frame_id);
        }
        else
        {
            break;
...
size_t get_tag_size(ID3v2_tag* tag)
{
	size_t size = 0;
...
void set_comment_frame(char* data, char encoding, ID3v2_frame* frame)
{
    ...
    frame->size = 1 + 3 + 1 + strlen(data); // encoding + language + description + comment
...
void set_album_cover_frame(char* album_cover_bytes, char* mimetype, size_t picture_size, ID3v2_frame* frame)
{
    char *frame_data;
    size_t offset;

    memcpy(frame->frame_id, ALBUM_COVER_FRAME_ID, 4);
    frame->size = 1 + strlen(mimetype) + 1 + 1 + 1 + picture_size; // encoding + mimetype + 00 + type + description + picture

    frame_data = (char*) malloc(frame->size * sizeof(char));
    frame->data = (char*) malloc(frame->size * sizeof(char));

    offset = 1 + strlen(mimetype) + 1 + 1 + 1;
...
void tag_set_album_cover_from_bytes(char* album_cover_bytes, char* mimetype, size_t picture_size, ID3v2_tag* tag)
{
