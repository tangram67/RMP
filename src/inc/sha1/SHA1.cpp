...
bool CSHA1::ReportHash(TCHAR* tszReport, REPORT_TYPE rtReportType) const
{
	...
	if( (rtReportType == REPORT_HEX) ||
		(rtReportType == REPORT_HEX_SHORT) ||
		(rtReportType == REPORT_HEX_NCASE) ||
		(rtReportType == REPORT_HEX_SHORT_NCASE) )
	{
		const TCHAR* lpFmt1;
		const TCHAR* lpFmtN;

		switch (rtReportType) {
			case REPORT_HEX_SHORT_NCASE:
				lpFmt1 = _T("%02x");
				lpFmtN = _T("%02x");
				break;
			case REPORT_HEX_NCASE:
				lpFmt1 = _T("%02x");
				lpFmtN = _T(" %02x");
				break;
			case REPORT_HEX_SHORT:
				lpFmt1 = _T("%02X");
				lpFmtN = _T("%02X");
				break;
			case REPORT_HEX:
		...
		_sntprintf(tszTemp, 15, lpFmt1, m_digest[0]);
		_tcscpy(tszReport, tszTemp);

		for(size_t i = 1; i < 20; ++i)
		{
			_sntprintf(tszTemp, 15, lpFmtN, m_digest[i]);
