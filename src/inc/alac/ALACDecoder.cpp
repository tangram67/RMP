...
// For some unknown reason Eclipse was missing the macro nil....
#ifndef nil
# define nil NULL
#endif
...
int32_t ALACDecoder::Init( void * inMagicCookie, uint32_t inMagicCookieSize )
{
    ...
    if (theCookieBytesRemaining >= sizeof(ALACSpecificConfig))
    {
        ...
        RequireAction( mConfig.compatibleVersion <= kALACVersion, return kALAC_ParamError_51; );

        // Set default frame size
        if (mConfig.frameLength <= 0)
        	mConfig.frameLength = kALACDefaultFrameSize;
...
int32_t ALACDecoder::Init( const ALACSpecificConfig& config )
{
	int32_t		status = ALAC_noErr;

	// Set the ALACSpecificConfig
	mConfig = config;

	RequireAction( mConfig.compatibleVersion <= kALACVersion,
			status = kALAC_ParamError_53; goto Exit; );
...
//	int16_t *			samples;
	...
	RequireAction( (bits != nil) && (sampleBuffer != nil) && (outNumSamples != nil), return kALAC_ParamError_54; );
	RequireAction( numChannels > 0, return kALAC_ParamError_55; );
...
//	samples = (int16_t *) sampleBuffer;
	...
	*outNumSamples = numSamples;
	...
	while ( status == ALAC_noErr )
	{
    	...
    	RequireAction( bits->cur < bits->end, status = kALAC_ParamError_56; goto Exit; );
		...
		switch ( tag )
		{
			...
			case ID_LFE:
			{
				...
				RequireAction( unusedHeader == 0, status = kALAC_ParamError_57; goto Exit; );
				...
				RequireAction( bytesShifted != 3, status = kALAC_ParamError_58; goto Exit; );
			...
			case ID_CPE:
			{
				...
				// read the 12 unused header bits
				unusedHeader = (uint16_t) BitBufferRead( bits, 12 );
				RequireAction( unusedHeader == 0, status = kALAC_ParamError_59; goto Exit; );
				...
				bytesShifted = (headerByte >> 1) & 0x3u;
				RequireAction( bytesShifted != 3, status = kALAC_ParamError_60; goto Exit; );
...
int32_t ALACDecoder::FillElement( BitBuffer * bits )
{
	...
	RequireAction( bits->cur <= bits->end, return kALAC_ParamError_62; );
...
//	element_instance_tag = BitBufferReadSmall( bits, 4 );
	...
	RequireAction( bits->cur <= bits->end, return kALAC_ParamError_63; );
