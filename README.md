...
The application is build in the ./build folder and installed in ./build/bin folder as "rmp". Pleas note that the rmp binary file wil loose the capability settings when copied to a different location. The root folder contains a batch file setcap.sh to restore all needed capabilities of the binary, execute as root, e.g. "sudo ./setcap /new/path/rmp".
